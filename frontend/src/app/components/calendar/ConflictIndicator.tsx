...
export const ConflictIndicator: React.FC<ConflictIndicatorProps> = memo(function ConflictIndicator({
  ...
  if (compact) {
    return (
      <span
        ...
        role="status"
        aria-label="Time slot conflict: Overlap"
      ...
      >
        <AlertCircle className="w-3 h-3 text-red-600 shrink-0" aria-hidden="true" />
        <span>Overlap</span>
      </span>
    );
  }
  ...
  return (
    <div role="alert" className="flex items-center gap-2 px-3 py-1.5 bg-red-50 border border-red-200 text-red-800 rounded-xl text-xs font-medium">
