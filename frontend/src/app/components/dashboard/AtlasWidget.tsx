...
import { AtlasFeatureBarricade } from '../atlas';
...
export const AtlasWidget: React.FC = memo(function AtlasWidget() {
  ...
  // When Atlas is disabled, prevent any interaction
  const isAtlasDisabled = !config.isAtlasMvpAvailable;
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm space-y-5">
        ...
        {activeTab === 'search' && (
          <div className="space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 max-h-48 overflow-y-auto space-y-2">
                {isSearching ? (
                  ...
                  searchResults.map((res) => (
                    <div
                      key={res.id}
                      onClick={() => !isAtlasDisabled && handleQuickRoute(res.latitude, res.longitude)}
            ...
            {isAtlasDisabled && searchQuery && (
              <div className="py-4 text-center text-xs text-gray-500 bg-slate-50 border border-slate-200 rounded-xl">
                Atlas Wayfinding is temporarily unavailable during MVP.
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'recent' && !isAtlasDisabled && (
        ...
        {isAtlasDisabled && activeTab === 'recent' && (
          <div className="text-center text-xs text-gray-500 py-8">
            <p>Atlas Wayfinding is temporarily unavailable during MVP.</p>
          </div>
        )}
      </div>
