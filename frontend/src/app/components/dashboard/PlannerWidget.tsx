...
import { Calendar, Target, Award, Plus, CheckCircle2, Clock, AlertCircle, Sparkles } from 'lucide-react';
...
export const PlannerWidget: React.FC = () => {
  ...
  const todaySchedule = timetable.filter((slot) => slot.dayOfWeek === currentDayStr);

  const completedGoalsCount = studyGoals.filter((g) => g.isCompleted).length;
            ...
            <div className="bg-white p-3.5 rounded-lg border border-gray-200 shadow-2xs mb-3">
              ...
              <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                <div
                  ...
                  style={{
                    width: `${Math.round(
                      ((degreePlan?.completedCredits || 78) / (degreePlan?.totalRequiredCredits || 120)) * 100
                    )}%`,
                  }}
                ></div>
              </div>
