...
export function NoticeDetailsModal({ notice, onClose, onEdit }: NoticeDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-gray-100">
        {/* Header */}
        <div className="p-6 pb-4 border-b border-gray-100 bg-gray-50/50 flex items-start justify-between gap-4">
          <div className="space-y-3 max-w-2xl">
            ...
            <h2 className="text-2xl font-bold text-gray-900 leading-snug">{notice.title}</h2>
          </div>

          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-200/60 rounded-xl transition-all shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex items-center gap-2">
            {onEdit && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-semibold text-xs rounded-xl transition-all"
              >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-semibold text-xs rounded-xl border border-red-200 transition-all"
            >
